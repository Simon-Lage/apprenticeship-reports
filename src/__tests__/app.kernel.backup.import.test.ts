...
describe('app kernel backup import', () => {
  ...
  it('accepts backup import previews without a source field', async () => {
    const { kernel } = createKernel();
    await kernel.boot();
    await signIn(kernel);
    await completeRequiredOnboarding(kernel);
    const baseState = createDefaultAppMetadata('2026-03-13T12:00:00.000Z');
    baseState.reports.weeklyReports['week-1'] = {
      id: 'week-1',
      weekStart: '2026-03-09',
      weekEnd: '2026-03-13',
      values: {},
    ...
    const withoutSource = { ...envelope };

    delete withoutSource.source;

    await expect(
      kernel.prepareBackupImport(JSON.stringify(withoutSource)),
  ...
  it('keeps local report data for conflicting weeks when local is selected', async () => {
    ...
    incomingState.reports.weeklyReports['backup-week-1'].dailyReportIds.push(
      'backup-day-3',
    );
    incomingState.reports.dailyReports['backup-day-3'] = {
      id: 'backup-day-3',
      weeklyReportId: 'backup-week-1',
      date: '2026-03-10',
      values: { source: 'backup-only-day' },
      createdAt: '2026-03-13T11:00:00.000Z',
      updatedAt: '2026-03-13T11:00:00.000Z',
    };
    ...
    const firstWeekDay = Object.values(
      persistedState.reports.dailyReports,
    ).find((dailyReport) => dailyReport.date === '2026-03-09');
    const conflictWeekExtraDay = Object.values(
      persistedState.reports.dailyReports,
    ...
    expect(conflictWeekExtraDay).toBeUndefined();
  ...
  it('uses backup report data for conflicting weeks when backup is selected', async () => {
    ...
    const localOnlyDay = Object.values(
      persistedState.reports.dailyReports,
    ).find((dailyReport) => dailyReport.date === '2026-03-10');
    ...
    expect(localOnlyDay).toBeUndefined();
    expect(Object.keys(persistedState.reports.weeklyReports)).toHaveLength(1);
  });

  it('uses the newest week timestamp by default for conflicts after a restart', async () => {
    const { kernel, repository } = createKernel();
    await kernel.boot();
    await signIn(kernel);
    await completeRequiredOnboarding(kernel);
    ...
    const restoredWeek = Object.values(
      restoredState.reports.weeklyReports,
    ).find(
      (weeklyReport) =>
        weeklyReport.weekStart === '2026-03-09' &&
        weeklyReport.weekEnd === '2026-03-13',
    );
    const restoredDayOne = Object.values(
      restoredState.reports.dailyReports,
    ).find((dailyReport) => dailyReport.date === '2026-03-09');
    const restoredDayTwo = Object.values(
      restoredState.reports.dailyReports,
    ).find((dailyReport) => dailyReport.date === '2026-03-10');
    ...
    expect(restoredWeek?.values).toMatchObject({
      source: 'backup',
    });
