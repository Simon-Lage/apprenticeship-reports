...
describe('onboarding progress', () => {
  ...
  it('uses the last active missing step as next step', () => {
    const progress = createOnboardingProgress('2026-03-13T10:00:00.000Z');
    const state = deriveOnboardingState(
      definitions,
      {
        ...progress,
        lastActiveStepId: 'company',
      },
      {},
    );

    expect(state.nextStepId).toBe('company');
  });
});
