...
import {
  ...
  BackupImportDecryptionInputSchema,
...
export default function registerAppHandlers(
  ...
  handleRendererError: (input: RendererErrorInput) => void,
): void {
  ...
  ipcMain.handle(AppIpcChannel.getAppBuildInfo, () => getAppBuildInfo());
  ipcMain.handle(AppIpcChannel.checkForUpdates, () => checkForUpdates());
  ...
  ipcMain.handle(AppIpcChannel.getPendingGoogleAuthorizationUrl, () =>
    appKernel.getPendingGoogleAuthorizationUrl(),
  );

  ipcMain.handle(AppIpcChannel.cancelPendingGoogleAuthorization, () =>
    appKernel.cancelPendingGoogleAuthorization(),
  );
  ...
  ipcMain.handle(AppIpcChannel.uploadBackupToDrive, (_event, input) =>
    appKernel.uploadBackupToDrive(ExportBackupArchiveInputSchema.parse(input)),
  );

  ipcMain.handle(AppIpcChannel.uploadSettingsBackupToDrive, (_event, input) =>
    appKernel.uploadSettingsBackupToDrive(
      ExportSettingsInputSchema.parse(input),
    ),
  );
  ...
  ipcMain.handle(AppIpcChannel.prepareDriveSettingsImport, (_event, input) =>
    appKernel.prepareDriveSettingsImport(
      PrepareDriveBackupImportInputSchema.parse(input),
    ),
  );
  ...
  ipcMain.handle(AppIpcChannel.exportSettings, (_event, input) =>
    appKernel.exportSettings(ExportSettingsInputSchema.parse(input)),
  );

  ipcMain.handle(
    AppIpcChannel.prepareSettingsImport,
    (_event, serialized, decryption) =>
      appKernel.prepareSettingsImport(
        String(serialized),
        BackupImportDecryptionInputSchema.parse(decryption),
      ),
  );
  ...
  ipcMain.handle(AppIpcChannel.exportBackupArchive, (_event, input) =>
    appKernel.exportBackupArchive(ExportBackupArchiveInputSchema.parse(input)),
  );

  ipcMain.handle(
    AppIpcChannel.prepareBackupImport,
    (_event, serialized, decryption) =>
      appKernel.prepareBackupImport(
        String(serialized),
        BackupImportDecryptionInputSchema.parse(decryption),
      ),
  );
  ...
  ipcMain.handle(AppIpcChannel.handleRendererError, (_event, input) => {
    handleRendererError(RendererErrorInputSchema.parse(input));
  });
}
