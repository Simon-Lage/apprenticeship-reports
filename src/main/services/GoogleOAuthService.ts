...
export class GoogleOAuthService {
  ...
  private pendingAuthorizationCancel: (() => void) | null = null;
  ...
  cancelPendingAuthorization(): void {
    this.pendingAuthorizationCancel?.();
  ...
  async authorize(
    ...
    this.pendingAuthorizationCancel = callback.cancel;

    try {
      await this.openExternal(this.pendingAuthorizationUrl);
      ...
      this.pendingAuthorizationUrl = null;
      this.pendingAuthorizationCancel = null;
  ...
  private async createLoopbackCallback(expectedState: string): Promise<{
    ...
    const waitForCode = () => authorizationCodePromise;
    ...
    return {
      ...
      cancel: () => {
        if (!settled && rejectCode) {
          settled = true;
          rejectCode(new Error('Google OAuth wurde abgebrochen.'));
        }
      },
      close: async () => {
        clearTimeout(timeout);
