...
export class PasswordAuthService {
  ...
  async initialize(password: PasswordAuthInput): Promise<void> {
    ...
    await this.repository.initializePassword(
      parsedPassword,
      createPasswordCredential(parsedPassword),
    );
  }

  async verify(password: PasswordAuthInput): Promise<boolean> {
    const parsedPassword = PasswordLoginSchema.parse(password);
    if (!PasswordSecretSchema.safeParse(parsedPassword).success) {
      return false;
    }
    ...
    return this.repository.unlockWithPassword(parsedPassword);
  }
