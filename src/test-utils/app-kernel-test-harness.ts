...
import { MemoryAppMetadataRepository } from '@/src/test-utils/MemoryAppMetadataRepository';

class TestSecretStorageService implements SecretStorageService {
...
export function useAppKernelTestHarness() {
  ...
  function createKernel(options?: {
    ...
    repositoryKind?: 'sqlite' | 'memory';
  }) {
    const repositoryPath = path.join(rootDirectory, 'app-metadata.json');
    const repository =
      options?.repositoryKind === 'memory'
        ? new MemoryAppMetadataRepository(
            repositoryPath,
            () => currentTime,
            ...
            .object({
              backup: z
                .object({
                  ...
                  reportsDailyThreshold: z.number().int(),
                  settingsEnabled: z.boolean(),
                  automaticBackupsEncrypted: z.boolean(),
                  exportSettingsOnboarding: z.boolean(),
                  exportSettingsUi: z.boolean(),
                  exportSettingsAbsence: z.boolean(),
                  automaticSettingsScope: z
                    .object({
  ...
  async function writeReportsFixture(
    ...
    state.backupEncryption = persistedState.backupEncryption;
