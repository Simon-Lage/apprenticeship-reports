...
import { parseCompanyLogoStepValues } from '@/shared/onboarding/company-logo';
...
export function parseOnboardingStepValues(
  ...
  if (stepId === 'company-logo') {
    return parseCompanyLogoStepValues(value) as JsonObject;
  }
...
export function getOnboardingStepDefaults(input: {
  ...
  if (input.stepId === 'company-logo') {
    return {
      dataUrl:
        typeof input.source.dataUrl === 'string' ? input.source.dataUrl : '',
    };
  }
