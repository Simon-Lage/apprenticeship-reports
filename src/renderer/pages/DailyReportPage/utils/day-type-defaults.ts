...
const sickReason = 'Krankheit';
const vacationReason = 'Urlaub';
...
export function resolveAutoDayTypeFromBase(input: {
  ...
  if (sickFromManual) {
    const label = sickFromManual.label.trim() || sickReason;
  ...
  if (vacationFromManual) {
    const label = vacationFromManual.label.trim() || vacationReason;
