  ...
  const renderDetailField = (
    ...
    return (
      <div
        key={field.label}
        className={cn(
          'min-h-[8mm] rounded-md border border-primary-tint/60 bg-primary-tint/10 px-2.5 py-1.5',
          options.compact
            ? 'grid grid-cols-[minmax(0,45%)_minmax(0,1fr)] items-start gap-2'
            : 'flex items-start gap-2',
        )}
      >
        <span className="min-w-0 text-[0.86em] font-semibold leading-snug text-text-color/75">
          {field.label}
        </span>
        <span className="min-w-0 break-words text-right font-medium leading-snug">
          {field.value}
        </span>
  ...
  return (
    ...
    >
      {pages.map((page, pageIndex) => (
        ...
        >
          <header className="relative shrink-0 space-y-2.5 border-b border-primary-tint/70 pb-3 pr-[34mm]">
            {document.companyLogoDataUrl ? (
              <img
                src={document.companyLogoDataUrl}
                alt=""
                className="absolute right-0 top-0 h-auto max-h-[16mm] max-w-[31mm] object-contain"
              />
            ) : null}
            <h2 className="whitespace-nowrap pl-[34mm] text-center text-[1.45em] font-semibold">
              {document.title}
            </h2>
            <div className="grid grid-cols-12 gap-1.5 text-[0.98em] [&>*:nth-child(1)]:col-span-3 [&>*:nth-child(2)]:col-span-3 [&>*:nth-child(3)]:col-span-6 [&>*:nth-child(4)]:col-span-4 [&>*:nth-child(5)]:col-span-4 [&>*:nth-child(6)]:col-span-4">
