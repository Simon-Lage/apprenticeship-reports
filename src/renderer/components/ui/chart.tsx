...
type ChartThemeConfig = Partial<Record<keyof typeof THEMES, string>>;
    ...
    >
      ...
      <div className="grid gap-1.5">
        {payload.map((item, index) => {
          ...
          const itemKey = `${item.dataKey ?? item.name ?? key}-${item.value ?? item.color ?? 'item'}`;
...
function getPayloadConfigFromPayload(
  ...
  } else if (typeof chartPayloadRecord?.[key] === 'string') {
    configLabelKey = chartPayloadRecord[key] as string;
  }
