...
const weeklyDocumentClipboardBullet = '•';
...
export function createWeeklyDocumentTranslations(
  ...
  return {
    ...
    pageLabel: t('weeklyDocument.pageLabel'),
    labels: {
...
function formatTrainingPeriod(
  ...
  const startValue = formatGermanDate(start);
  const endValue = formatGermanDate(end);
...
export function serializeWeeklyDocumentSectionEntries(
  input: WeeklyDocumentSectionEntry[],
): string {
  return input
    .map((entry) =>
      [
        entry.heading,
        ...entry.items.map(
          (item) => `${weeklyDocumentClipboardBullet} ${item}`,
        ),
      ].join('\n'),
    )
    .join('\n\n');
...
export function buildWeeklyDocumentSections(input: {
  dailyReports: DailyReportRecord[];
  translations: WeeklyDocumentTranslations;
}): WeeklyDocumentSection[] {
  const groups = buildWeeklySectionDayGroups(input.dailyReports);
...
function resolveDensity(
  sections: WeeklyDocumentSection[],
): WeeklyDocumentDensity {
  const itemCount = sections.reduce(
    (sum, section) =>
      sum +
      section.entries.reduce(
        (entrySum, entry) => entrySum + entry.items.length,
        0,
      ),
    0,
  );
...
export function buildWeeklyDocumentData(input: {
  ...
  const companyLogo = parseOnboardingCompanyLogo(input.settingsValues);
  ...
  const supervisorValue =
    ...
    '';
  const supervisorRepeatValue = supervisorValue;
  const areaValue =
    parsedWeek.area ||
    workplace.department ||
    uiSettings.defaultDepartment ||
    '';
  const sections = buildWeeklyDocumentSections({
    dailyReports: input.week.dailyReports,
    translations: input.translations,
  });

  return {
    title: `${input.translations.title} ${formatGermanDate(
    ...
    rangeStartField: {
      ...
      value: resolveValue(
        formatGermanDate(input.week.weeklyReport.weekStart),
        input.translations.emptyValue,
      ),
    },
    rangeEndField: {
    ...
    supervisorRepeatField: {
      ...
      value: resolveValue(supervisorRepeatValue, input.translations.emptyValue),
    },
...
function buildCompanyLogoHtml(dataUrl: string | null): string {
  if (!dataUrl) {
    return '';
  }

  return `<img class="company-logo" src="${escapeHtml(dataUrl)}" alt="" />`;
}
...
<body>
  <main class="page ${input.document.density}">
    <header class="document-header">
      ${buildCompanyLogoHtml(input.document.companyLogoDataUrl)}
