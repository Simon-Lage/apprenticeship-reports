...
export function resolveAbsenceCatalogStartYear(
  values: JsonObject,
): number | null {
  const baselineDate = resolveReportStartDateFromSettings(values);

  return baselineDate ? Number(baselineDate.slice(0, 4)) : null;
}

export function resolveRequiredAbsenceCatalogYears(input: {
  values: JsonObject;
  currentYear: number;
  fallbackStartYear?: number | null;
}): number[] {
  const resolvedStartYear =
    resolveAbsenceCatalogStartYear(input.values) ??
    input.fallbackStartYear ??
    input.currentYear;
  const boundedStartYear = Math.min(resolvedStartYear, input.currentYear);
  const years: number[] = [];

  for (let year = boundedStartYear; year <= input.currentYear; year += 1) {
    years.push(year);
  }
...
export function getMissingAbsenceCatalogYears(input: {
  absence: AbsenceSettings;
  subdivisionCode: string;
  requiredYears: number[];
}): number[] {
  return input.requiredYears.filter((year) => {
    const catalog = input.absence.catalogsByYear[createCatalogYearKey(year)];

    if (!catalog) {
      return true;
    }

    return catalog.subdivisionCode !== input.subdivisionCode;
  });
}
