...
function assertValidWeekRange(weekStart: string, weekEnd: string): void {
  if (weekEnd < weekStart) {
    throw new Error(
      'Invalid week range: weekEnd must not be before weekStart.',
    );
  }
}
...
function assertDateWithinWeek(
  ...
  if (date < weekStart || date > weekEnd) {
    throw new Error(
      'Daily report date must be within the selected week range.',
    );
  }
}
...
export function applyUpsertDailyReport(
  ...
  const nextDailyReport = DailyReportRecordSchema.parse({
    id:
      currentDailyReport?.id ?? createDayId(currentWeeklyReport.id, input.date),
