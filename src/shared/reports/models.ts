...
export function createWeeklyReportContentHash(input: {
  ...
  const weekValuesForHash = Object.entries(weekValues).reduce<JsonObject>(
    (result, [key, value]) => {
      if (key === 'submitted' || key === 'submittedToEmail' || key === 'area') {
        return result;
      }
...
export function summarizeReportConflicts(
  ...
  incomingWeeksByIdentity.forEach((incomingWeeklyReport, identity) => {
    ...
    const currentDailyReportsByDate =
      mapDailyReportsByDate(currentDailyReports);
    const incomingDailyReportsByDate =
      mapDailyReportsByDate(incomingDailyReports);
...
export function mergeReportsState(input: {
  ...
  incomingWeeksByIdentity.forEach((incomingWeeklyReport, identity) => {
    ...
    const useIncomingWeek =
      strategyForWeek === 'backup' ||
      (strategyForWeek === 'latest-timestamp' &&
        incomingWeeklyReport.updatedAt > currentWeeklyReport.updatedAt);
    const selectedWeeklyReport = useIncomingWeek
      ? incomingWeeklyReport
      : currentWeeklyReport;
    const selectedDailyReports = getDailyReportsForWeek(
      useIncomingWeek ? incomingState.dailyReports : currentState.dailyReports,
      selectedWeeklyReport,
    );

    nextWeeklyReports[selectedWeeklyReport.id] = selectedWeeklyReport;
    selectedDailyReports.forEach((dailyReport) => {
      nextDailyReports[dailyReport.id] = dailyReport;
    });
  });
