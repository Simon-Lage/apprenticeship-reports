...
export const DatabaseBackupDaySchema = z
  .object({
    date: z.string().date(),
...
export const DatabaseBackupWeekSchema = z
  .object({
...
export const DatabaseBackupReportsSchema = z
  .object({
    weeks: z.array(DatabaseBackupWeekSchema).min(1),
  })
  .strict();

export const DatabaseBackupEnvelopeSchema = z
  .object({
    exportedAt: z.string().datetime(),
    source: z.literal('reports-json').optional(),
  ...
  envelope.reports.weeks.forEach((week) => {
    ...
    week.days.forEach((day) => {
      if (dayDateSet.has(day.date)) {
        throw new Error(
          `Duplicate day ${day.date} in backup week ${identity}.`,
        );
      }
...
export function createReportsStateFromDatabaseBackup(
  ...
  envelope.reports.weeks.forEach((week) => {
    ...
    reportsState.weeklyReports[weekId] = WeeklyReportRecordSchema.parse({
      ...
      weekEnd: week.weekEnd,
      values: normalizeWeekValues(week),
      dailyReportIds: dailyReports.map((day) =>
        createInternalDayId(weekId, day.date),
      ),
      createdAt: week.updatedAt,
      updatedAt: week.updatedAt,
    });
...
function createWeeksByIdentity(
  ...
  Object.values(reports.weeklyReports).forEach((weeklyReport) => {
    ...
    weeksByIdentity.set(createWeekIdentity(week.weekStart, week.weekEnd), week);
  });
